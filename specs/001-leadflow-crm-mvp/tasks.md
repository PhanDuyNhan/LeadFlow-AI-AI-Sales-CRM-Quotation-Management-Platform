...
- [X] T017 [P] Create `server/src/models/Quotation.js` — Mongoose schema with embedded items (`name`, `description`, `quantity ≥ 0`, `unitPrice ≥ 0`, computed `totalPrice`); top-level `quotationCode` (unique), `lead` (ref Lead, required), `customerName`, `subtotal`, `discount ≥ 0`, `tax ≥ 0`, computed `totalAmount`, `status` (enum `Draft|Sent|Accepted|Rejected|Expired`, default `Draft`), `validUntil`, `notes`, `createdBy` (ref User), timestamps; pre-validate hook computes line totals + subtotal + totalAmount; indexes on `quotationCode` (unique), `{lead}`, `{createdBy,status}`.
- [X] T018 [P] Create `server/src/models/Task.js` — Mongoose schema: `title` (required), `description`, `dueDate` (Date, required), `priority` (enum `Low|Medium|High`, default `Medium`), `status` (enum `Pending|Completed|Overdue`, default `Pending`), `completedAt` (Date), `lead` (ref Lead, optional), `createdBy` (ref User, required), `assignedTo` (ref User), timestamps; indexes on `{createdBy,status,dueDate}`, `{assignedTo,status}`, `{lead}`
...
- [X] T084 [US4] Create `server/src/validators/quotation.validator.js` — express-validator chains for create/update/status/list with required/optional rules: `lead` required+ObjectId, `items` non-empty array, `quantity`/`unitPrice` non-negative, `discount`/`tax` non-negative, status enum, validUntil ISO date, leadStatus enum on status changes.
- [X] T085 [US4] Create `server/src/services/quotation.service.js` — `listQuotations`, `listByLead`, `getQuotationById`, `createQuotation` (auto-generates `quotationCode` when omitted, enforces uniqueness, role-scoped lead access check), `updateQuotation` (blocks item edits when status is Accepted), `updateStatus` (optionally propagates Accept→Won / Reject→Negotiating|Lost to the linked lead), `deleteQuotation` (Draft-only + owner/admin), `generateUniqueCode`. Scoping: admin sees all; user sees quotations they created or quotations whose linked lead they own/are assigned to.
- [X] T086 [US4] Create `server/src/controllers/quotation.controller.js` — controllers delegate to the service and use the shared `{ success, message, data }` envelope.
- [X] T087 [US4] Create `server/src/routes/quotation.routes.js` — `GET /`, `GET /generate-code`, `GET /by-lead/:leadId`, `POST /`, `GET /:id`, `PUT /:id`, `PATCH /:id/status`, `DELETE /:id`; all protected. (Single `PATCH /:id/status` covers Send/Accept/Reject + optional `leadStatus` rather than a dedicated `/accept` endpoint.)
- [X] T088 [US4] Mount quotation routes in `server/server.js`: `app.use('/api/quotations', quotationRoutes)`. Also added `GET /api/leads/minimal` to populate the lead dropdown.
- [X] T089 [US4] Create `client/src/services/quotation.service.js` — `listQuotations`, `listByLead`, `getQuotation`, `createQuotation`, `updateQuotation`, `updateQuotationStatus`, `deleteQuotation`, `generateCode`, `listLeadsMinimal`.
- [ ] T090 [P] [US4] *(deferred — state lives inline in pages; no separate hook needed yet)* Create `client/src/hooks/useQuotations.js`.
- [X] T091 [US4] Create `client/src/components/quotations/LineItemEditor.jsx` — dynamic line-item rows (name / description / quantity / unitPrice); live row total + subtotal + grand total via `useMemo`; add/remove row buttons; locks all inputs when `isLocked` is true; embedded discount + tax inputs.
- [X] T092 [US4] Create `client/src/components/quotations/QuotationForm.jsx` — lead selector populated by `listLeadsMinimal()`, auto-suggested `quotationCode` from `generateCode()`, customerName auto-fill, validUntil, status (edit mode), notes, LineItemEditor; client-side validation for required fields + non-negative numbers.
- [X] T093 [P] [US4] Create `client/src/components/quotations/QuotationTable.jsx` — columns: code, customer/lead, status (Badge), totalAmount, validUntil, createdAt, Actions; delete button disabled when not Draft.
- [X] T094 [US4] Create `client/src/pages/QuotationsPage.jsx` — search + status filter + pagination + QuotationTable; delete via ConfirmDialog.
- [X] T095 [US4] Create `client/src/pages/QuotationCreatePage.jsx` — renders QuotationForm; pre-selects `?lead=<id>` if present (used by the lead detail page); on success → `/quotations/:id`.
- [X] T096 [US4] Create `client/src/pages/QuotationDetailPage.jsx` — full details + line item table + totals; Edit toggles inline form; Send button (Draft → Sent); Accept dialog with "mark lead as Won" checkbox; Reject dialog with optional Negotiating/Lost lead status; Delete via ConfirmDialog (Draft-only).
- [X] T097 [US4] Add quotation routes to `client/src/routes/AppRoutes.jsx`: `/quotations`, `/quotations/create`, `/quotations/:id`. Sidebar nav link added in `AppLayout.jsx`. `RelatedQuotations` section rendered on the lead detail page.
...
- [X] T104 [US5] Create `server/src/validators/task.validator.js` — express-validator: `title` required, `dueDate` required ISO date, `priority` optional enum (`Low|Medium|High`), `status` optional enum (`Pending|Completed|Overdue`), `lead`/`assignedTo` optional ObjectId; list query rules include `filter`, `status`, `priority`, `search`.
- [X] T105 [US5] Create `server/src/services/task.service.js` — `listTasks` (filters: `today`/`overdue`/`pending`/`completed`/`all` + `status`/`priority`/`search`); `getTodayFollowUps` returns today's tasks **and** leads whose `nextFollowUpDate` is today; `getOverdueFollowUps`; `createTask`; `getTaskById`; `updateTask`; `completeTask`; `deleteTask`. Role scoping: admin sees all; user sees tasks they created or are assigned. Delete restricted to admin or creator. Sweeps `status → Overdue` on any list call (dueDate `< today` AND status `≠ Completed`).
- [X] T106 [US5] Create `server/src/controllers/task.controller.js` — `listTasks`, `todayFollowUps`, `overdueFollowUps`, `getTask`, `createTask`, `updateTask`, `completeTask`, `deleteTask` delegate to the service with the shared `{ success, message, data }` envelope.
- [X] T107 [US5] Create `server/src/routes/task.routes.js` — `GET /`, `GET /today`, `GET /overdue`, `POST /`, `GET /:id`, `PUT /:id`, `PATCH /:id/complete`, `DELETE /:id`; all protected by `auth.middleware`.
- [X] T108 [US5] Mount task routes in `server/server.js`: `app.use('/api/tasks', taskRoutes)`.
- [X] T109 [US5] Create `client/src/services/task.service.js` — `listTasks`, `getTodayFollowUps`, `getOverdueFollowUps`, `getTask`, `createTask`, `updateTask`, `completeTask`, `deleteTask`, `listLeadsMinimal`.
- [ ] T110 [P] [US5] *(deferred — state lives inline in TasksPage; no separate hook needed yet)* Create `client/src/hooks/useTasks.js`.
- [X] T111 [US5] Create `client/src/components/tasks/TaskForm.jsx` — fields: `title` (required), `description`, `dueDate` (date picker, required), `priority` (Low/Medium/High), `status` (edit mode), `lead` selector via `listLeadsMinimal()`; renders inside `TaskModal.jsx`.
- [X] T112 [US5] Create `client/src/components/tasks/TaskList.jsx` — reusable card list with status & priority badges, due date, linked lead link, "Mark complete" button, Edit + Delete actions; loading / error / EmptyState handling.
- [X] T113 [US5] Create `client/src/pages/TasksPage.jsx` — tabs for Today / Overdue / All tasks; today tab also shows leads with `nextFollowUpDate = today`; "New task" button opens TaskForm in modal; Edit opens modal with pre-filled form; complete + delete with toast notifications; ConfirmDialog for delete.
- [X] T114 [US5] Add `/tasks` route to `client/src/routes/AppRoutes.jsx` and "Tasks" nav link in `AppLayout.jsx`.
...
- [X] T120 [US6] Create `server/src/services/dashboard.service.js` — `getDashboard(user)` returns `{ summary, charts, topHotLeads, todayFollowUps, overdueFollowUps }`. Summary: `totalLeads`, `newLeads`, `hotLeads`, `warmLeads`, `coldLeads`, `wonLeads`, `totalQuotations`, `sentQuotations`, `acceptedQuotations`, `estimatedRevenue` (sum of `totalAmount` where status ∈ {Sent, Accepted}), `conversionRate` (`wonLeads / totalLeads * 100`, 1 decimal, zero-safe), `followUpsToday` (tasks due today + leads with `nextFollowUpDate` today), `followUpTasksToday`, `followUpLeadsToday`, `overdueFollowUps`. Charts: `leadByStatus`, `leadBySource`, `quotationByStatus` (with totalAmount per status), `revenueForecast` (last 6 months grouped by createdAt month, Sent + Accepted). Role scoping: admin sees all; user sees leads/tasks they created or are assigned to, and quotations they created or whose linked lead is visible.
- [X] T121 [US6] Create `server/src/controllers/dashboard.controller.js` — `getDashboard` returns full payload; plus per-section controllers (`getSummary`, `getLeadByStatus`, `getLeadBySource`, `getQuotationStatusStats`, `getRevenueForecast`, `getTopHotLeads`, `getTodayFollowUps`, `getOverdueFollowUps`).
- [X] T122 [US6] Create `server/src/routes/dashboard.routes.js` — all protected: `GET /`, `GET /summary`, `GET /lead-by-status`, `GET /lead-by-source`, `GET /quotation-status`, `GET /revenue-forecast`, `GET /top-hot-leads`, `GET /follow-ups/today`, `GET /follow-ups/overdue`.
- [X] T123 [US6] Mount dashboard routes in `server/server.js`: `app.use('/api/dashboard', dashboardRoutes)`.
- [X] T124 [US6] Create `client/src/services/dashboard.service.js` — `getDashboard`, `getSummary`, `getLeadByStatus`, `getLeadBySource`, `getQuotationStatusStats`, `getRevenueForecast`, `getTopHotLeads`, `getTodayFollowUps`, `getOverdueFollowUps`.
- [X] T125 [US6] Create `client/src/components/dashboard/MetricCard.jsx` — color-tinted icon, title, value, optional hint sub-label; plus `ChartCard.jsx` wrapper with empty-state.
- [X] T126 [P] [US6] Create `client/src/components/dashboard/charts/LeadByStatusChart.jsx` — Recharts `BarChart` with per-status color cells; responsive container.
- [X] T127 [P] [US6] Create `client/src/components/dashboard/charts/LeadBySourceChart.jsx` — Recharts donut `PieChart` with palette per slice; legend; responsive container.
- [X] T128 [P] [US6] Create `client/src/components/dashboard/charts/QuotationStatusChart.jsx` — Recharts `BarChart` with per-status color cells; responsive container.
- [X] T129 [P] [US6] Create `client/src/components/dashboard/charts/RevenueForecastChart.jsx` — Recharts `AreaChart` with Sent vs Accepted series, last 6 months on x-axis, compact-currency y-axis.
- [X] T130 [US6] Create `client/src/pages/DashboardPage.jsx` — fetches `/dashboard` on mount; renders 12 metric cards (responsive 2/3/4-col grid); 4 charts; today's follow-ups widget (tasks + leads); overdue follow-ups widget; top hot leads table. Loading spinner, error retry, and global empty state when no data exists.
- [X] T131 [US6] Set `/dashboard` as the default redirect after login in `client/src/routes/AppRoutes.jsx` *(already in place — `/` redirects to `/dashboard` and `LoginPage` navigates there on success).*
